...
class Controller:
    ...
    def sense_compute_and_actuate(self):
        if(len(self.inputs) > 0 and len(self.inputsPrevious) > 0):
            ...
            def avoid():
                if(np.max([self.inputs[4],self.inputs[10]]) > 0.03):
                    # Time
                    time = datetime.now()
                    self.flag_turn = 1
                    print("({}) Object or walls detected!".format(np.argmax(self.inputs[3:11])))
            def avoid1():
                p_sensor=np.max(self.inputs[3:11])
                if(p_sensor > 0.03):
                    sensoridx=np.argmax(self.inputs[3:11])
                    time = datetime.now()
                    if(sensoridx<2):
                        print("({}) Object or walls detected!".format("R"))
                        self.velocity_right = 0.5;
                        self.velocity_left = 0.4;
                    elif(sensoridx>5):
                        self.velocity_left =0.5;
                        self.velocity_right = 0.4;
                        print("({}) Object or walls detected!".format("L"))
                    else :
                        self.velocity_left = 1;
                        self.velocity_right = 1;    
                        print("({}) Object or walls detected!".format("S"))
            avoid1()
